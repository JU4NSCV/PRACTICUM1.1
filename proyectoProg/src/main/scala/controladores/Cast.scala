...
object Cast extends App {
  ...
  try {
    ...
    val movieActorsList = new ListBuffer[(Int, Int, String, Int, Int, String,String)]()
      ...
      for {
        ...
        jsonObj.validate[Cast] match {
          case JsSuccess(cast, _) =>
            ...
            movieActorsList += ((movieId, cast.id, cast.character, cast.order, cast.cast_id, cast.credit_id,cast.profile_path.toString))
          case JsError(errors) =>
    ...
    if (INSERTCAST.InsertIntoCast(movieActorsList.toList))
      println("CORRECTO")
    else
      println("ERROR")
